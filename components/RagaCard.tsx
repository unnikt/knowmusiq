...
export default function RagaCard({
    ...
    return (
        <div className=" rounded-xl" >
            ...
            {description && (
                <p
                    className={`mt-3 max-h-90 leading-relaxed border-b 
                        ${expDescription ? "overflow-y-auto text-(--primary) scale-105 px-4 w-[90vw] mx-auto" : "text-sm text-(--primary)/60 line-clamp-2"} ${minRaga ? "hidden" : ""}`}
            ...
            <div className="flex justify-between items-center mt-2 border-t-2 pt-2 border-my-secondary">
                ...
                <AddVideo slug={slug} name={name} tag={"raga"} onSaved={onSaved} src={`/raga/${slug}`} />
            </div>
